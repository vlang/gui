...
v-gui is a flex-box style UI framework written in [V](https://vlang.io),
with declarative syntax and performance. If that sounds like
the Clay layout library, you\'re not mistaken. v-gui is based on the same
...
v-gui\'s rendering mechanism is immediate mode. That means that any change
...
opposite. In theory, v-gui can draw 10,000 frames a second. Of course,
hardware limitations and other factors limit performance, but
practically speaking, v-gui is faster than you\'ll ever need it to be.
...
v-gui is platform-independent and should run on the same platforms the V
supports.
